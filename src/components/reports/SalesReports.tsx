...
const RevenueChartTooltip = ({
  ...
  if (active && payload && payload.length) {
    return <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700">
...
const RevenueByPeriodChart = ({
  ...
  return <ResponsiveContainer width="100%" height="100%">
      ...
      bottom: 0
        ...
        <Area type="monotone" dataKey="valor" stroke="#8884d8" fill="#c7d2fe" strokeWidth={2} name="Faturamento" />
      </AreaChart>
    </ResponsiveContainer>;
};
const ServiceRevenueChart = ({
  ...
  return <ResponsiveContainer width="100%" height="100%">
      ...
      bottom: 70
        ...
        <XAxis dataKey="name" fontSize={12} stroke="#9ca3af" angle={-45} textAnchor="end" height={70} tickMargin={20} />
        <YAxis fontSize={12} stroke="#9ca3af" tickFormatter={value => formatCurrency(value)} />
        <Tooltip content={<ServiceChartTooltip />} />
        <Bar dataKey="valor" fill="#a7f3d0" radius={[4, 4, 0, 0]} stroke="#10b981" strokeWidth={1} name="Faturamento" />
      </BarChart>
    </ResponsiveContainer>;
};
const TopServicesChart = ({
  ...
  return <ResponsiveContainer width="100%" height="100%">
      <BarChart data={[...data].reverse()} layout="vertical" margin={{
      top: 10,
      right: 30,
      left: 100,
      bottom: 5
        ...
        <YAxis type="category" dataKey="service" fontSize={12} stroke="#9ca3af" width={80} tickFormatter={value => value.length > 15 ? `${value.substring(0, 15)}...` : value} />
        <Tooltip content={<ServiceChartTooltip />} />
        <Bar dataKey="revenue" fill="#bfdbfe" radius={[0, 4, 4, 0]} stroke="#3b82f6" strokeWidth={1} name="Faturamento" />
      </BarChart>
    </ResponsiveContainer>;
};
const ServiceDetailChart = ({
  data
}: {
  data: Array<{
    service: string;
  ...
  return <ResponsiveContainer width="100%" height="100%">
      <PieChart>
        <Pie data={data} cx="50%" cy="50%" labelLine={false} outerRadius={80} innerRadius={30} fill="#8884d8" dataKey="revenue" nameKey="service" label={({
        service,
        percent
...
const SalesReports: React.FC<SalesReportsProps> = ({
  ...
  const bentoCards = [{
    ...
    background: <KpiCardContent value={formatCurrency(data.comparison.current)} change={data.comparison.percentage} prevValue={formatCurrency(data.comparison.previous)} />,
    className: 'md:col-span-1 md:row-span-1'
  }, {
    Icon: CreditCard,
    name: 'Ticket Médio',
    description: 'Valor médio por serviço',
    background: <KpiCardContent value={formatCurrency(data.ticketAverage)} change={0} isPositive={true} />,
    className: 'md:col-span-1 md:row-span-1'
  }, {
    Icon: Wrench,
    name: 'Serviços Realizados',
    description: 'Quantidade de serviços no período',
    background: <KpiCardContent value={data.byPeriod.length.toString()} change={0} isPositive={true} />,
    className: 'md:col-span-1 md:row-span-1'
  }, {
    Icon: LineChart,
    name: 'Faturamento por Período',
    description: 'Evolução do faturamento no período selecionado',
    background: <div className="h-full w-full p-4">
  ...
  return <BentoGrid className="md:grid-cols-3">
      {bentoCards.map(card => <BentoCard key={card.name} cta="" onCtaClick={() => {}} className="px-0 py-0 my-0 mx-[9px]" />)}
    </BentoGrid>;
};
